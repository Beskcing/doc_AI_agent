...
排版完成后，在任务详情页点击「快速审查」按钮，系统基于规则引擎对文档全文进行快速扫描，检测以下问题：

- Unicode 非法字符（乱码、替换符等）
- 全角/半角混用
- 多余空格/空行
- 标点符号错误
- LaTeX 公式残留
...
审查结果以标记版 HTML 形式展示，问题逐条列出，支持一键修正。

### 9.2 深度审查

点击「深度审查」按钮，系统调用 LLM 对文档全文进行逐块审查（每块约 8000 字符），检测规则无法覆盖的语义级问题：
...
- **单条修正**：点击问题旁的修正按钮，AI 自动修正该问题
- **批量修正**：一键修正所有问题
- **重新审查**：修正后可重新审查确认

> **注意**：修正操作会清除当前审查缓存，下次查看标记版 HTML 时自动重新生成。
...
管理员登录后，在左侧菜单「用户管理」页面可管理所有用户账号：

- **查看用户列表**：分页展示所有用户（用户名/角色/状态/创建时间）
- **创建用户**：点击「创建用户」按钮，填写用户名/密码/角色
- **编辑用户**：修改角色、重置密码、启用/禁用
- **删除用户**：级联删除用户的所有数据（任务/对话/模板/文件）
