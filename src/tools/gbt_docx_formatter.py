...
将 format_gbt.py 的硬编码 GB/T 1.1 规范封装为可直接调用的格式化工具。
...
class GbtDocxFormatter:
    ...
    def _apply_cover_format(self, paragraph, text: str, para_idx: int) -> None:
        ...
        for run in paragraph.runs:
            ...
            if "国家标准" in text_stripped:
                ...
                self._set_run_font(
                    rPr, east_asia="宋体", latin="Times New Roman", size_pt=16.0, bold=True, bold_cs=True
                )
    ...
    def _apply_pre_cover_title_format(self, paragraph) -> None:
        ...
        for run in paragraph.runs:
            rPr = self._get_or_add_rPr(run)
            self._set_run_font(rPr, east_asia="宋体", latin="Times New Roman", size_pt=16.0, bold=True, bold_cs=True)
        self._set_para_format(
            paragraph,
    ...
    def _apply_heading_format(self, paragraph) -> None:
        ...
        for run in paragraph.runs:
            rPr = self._get_or_add_rPr(run)
            self._set_run_font(rPr, east_asia="宋体", latin="Times New Roman", size_pt=10.5, bold=False, bold_cs=False)
        self._set_para_format(
            paragraph,
            alignment=WD_ALIGN_PARAGRAPH.JUSTIFY,
            first_line_indent_pt=0,
