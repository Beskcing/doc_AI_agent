"""Loop Engineering V5 全面真实串联测试

将所有功能和按钮真实串联使用，覆盖每一个 API 端点和业务流程。
新增覆盖项（相比 V4）：
  - 模板上传提取 (POST /api/templates/upload)
  - 上传修正DOCX (POST /api/tasks/{id}/upload-corrected-docx) 功能1
  - 原始PDF预览 (GET /api/tasks/{id}/preview/original-pdf)
  - 知识库完整生命周期 (上传→重建→检索→删除)
  - 保存样式到已有模板 (update existing)
  - HTML格式内容更新
  - 多轮对话样式修改 (2+ turns)
  - 内容编辑Markdown模式串联DOCX重新生成
  - 任务重试→取消→删除完整链路
"""
...
def test_02_config():
    """T02: 系统配置（GET/PUT/standards/llm-models 全覆盖）"""
    print("\n═══ T02: 系统配置 ═══")
    try:
        # 获取配置
...
def test_06_content_edit(task_id: str, status: str):
    """T06: 内容编辑全流程（获取MD/获取HTML/更新MD/更新HTML）"""
    print("\n═══ T06: 内容编辑全流程 ═══")
    if status != "completed":
        print("  ⏭️ 跳过（任务未完成）")
        return
    ...
    try:
        ...
        # 更新 Markdown 内容（在末尾添加标记）
        test_content = content + "\n\n## Loop Engineering V5 测试标记\n\n此为自动化测试添加的内容。"
        ...
        # 【新增】更新 HTML 内容（模拟富文本编辑器保存）
        simple_html = "<h1>测试标题</h1><p>这是Loop Engineering V5 HTML内容编辑测试。</p>"
        ...
        if "Loop Engineering" in saved or "测试标题" in saved:
...
def test_09_apply_template(task_id: str, status: str, template_id: str):
    ...
    try:
        ...
        # 【新增】用 style_config 直接应用（不通过template_id）
        direct_style = {
...
def test_10_save_style_to_template(task_id: str, status: str, template_id: str):
    ...
    try:
        ...
        r2 = requests.post(f"{BASE}/api/tasks/{task_id}/save-style-to-template", json={
            "template_name": "V5-从任务保存",
            "style_config": style_config,
            "description": "V5自动化测试：从任务保存",
        })
        d2 = r2.json()
        if d2["code"] == 0:
        ...
        r3 = requests.post(f"{BASE}/api/tasks/{task_id}/save-style-to-template", json={
            "template_id": template_id,
            "template_name": "V5测试模板(任务回写更新)",
            "style_config": style_config,
            "description": "V5自动化测试：更新已有模板",
        })
        d3 = r3.json()
        if d3["code"] == 0:
...
def test_11_upload_corrected_docx(task_id: str, status: str):
    ...
    try:
        ...
        # 保存到临时文件
        tmp_path = Path("data/uploads/v5_corrected_test.docx")
...
def test_13_chat_content(task_id: str, status: str):
    ...
    try:
        r = requests.post(f"{BASE}/api/chat/content", json={
            "message": "在文档末尾添加一行：本文档由 Loop Engineering V5 自动化测试生成。",
            "task_id": task_id,
        }, timeout=120)
        d = r.json()
        if d["code"] == 0:
            ...
            if "Loop Engineering V5" in updated_md:
...
def test_14_kb_full_cycle():
    """T14: 知识库完整生命周期（统计→列表→上传→重建→检索→删除）"""
    print("\n═══ T14: 知识库完整生命周期 ═══")
    created_doc_id = None
    try:
        # KB 统计
        ...
        # 上传知识库文档
        test_md = "# 测试知识库文档\n\n这是V5测试上传的知识库文档，包含排版规范相关内容。\n\n## 字体要求\n\n正文使用仿宋_GB2312。"
        r3 = requests.post(f"{BASE}/api/kb/documents", files={
            "file": ("v5_test_kb_doc.md", test_md.encode("utf-8"), "text/markdown"),
        })
        d3 = r3.json()
        if d3["code"] == 0:
...
def main():
    print("=" * 70)
    print("🔄 Loop Engineering V5 全面真实串联测试")
